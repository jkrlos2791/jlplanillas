...
class EmpresaController extends Controller {
	...
	public function getIndex( Request $request )
	{
		...
		return view('empresa.index',$this->data);
	}	
	...
	function postSave( Request $request)
	{
		...
		if ($validator->passes()) {
		    ...
		    $itdmodelos = Concepto::where('empresa_id','=',0)->get();
			
			//return dd($itdmodelos)
			
			foreach($itdmodelos as $itdmodelo){
			
			$itd = new Concepto();
            $itd->empresa_id = $id;
            $itd->tipo  = $itdmodelo->tipo;
			$itd->codigo  = $itdmodelo->codigo;
			$itd->descripcion  = $itdmodelo->descripcion;
			$itd->estado  = $itdmodelo->estado;
			$itd->habilitado  = $itdmodelo->habilitado;
            $itd->save();
			}
	...
	{
		if(Session::get('empresa_id')!=0 && Session::get('empresa_id')!=null){
		    return Redirect::to('empresa')->with('mensaje', '<p class="alert alert-success">'."Debes salir de la sesión".'</p>');
        }
        else{
