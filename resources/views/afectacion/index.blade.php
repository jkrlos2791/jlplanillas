  ...
  <div class="page-content row">
	...
	<div class="sbox-title">
		<div class="sbox-tools pull-left" >
			...
			@if($access['is_add'] ==1)
	   		<a href="{{ URL::to('afectacion/update?return='.$return) }}" class="tips btn btn-xs btn-success btn-sm btn-circle"  title="{{ Lang::get('core.btn_create') }}">
			...
			@endif 
			<a href="{{ URL::to( 'afectacion/search?return='.$return) }}" class="btn btn-xs btn-warning btn-sm btn-circle" onclick="SximoModal(this.href,'Advance Search'); return false;" title="{{ Lang::get('core.btn_search') }}"><i class="fa  fa-search"></i> </a>				
		...
		<div class="sbox-tools" >
			...
			<a href="{{ URL::to('afectacion/download?return='.$return) }}" class="tips btn btn-xs btn-primary btn-sm btn-circle" title="{{ Lang::get('core.btn_download') }}">
	...
	<div class="sbox-content"> 	
	 ...
	 {!! Form::open(array('url'=>'afectacion/delete/0?return='.$return, 'class'=>'form-horizontal' ,'id' =>'SximoTable' )) !!}
            ...
            @foreach ($rowData as $row)
                <tr style="height: 40px">
					...
					<td width="100">
						...
						<input class="habilitado" value="{{ $row->habilitado }}" name="habilitado[]" id = "habilitado_java_{{$row->itd_id}}" type="checkbox" data-toggle="toggle" data-style="ios" data-itd-id="{{ $row->itd_id }}" data-habilitado="{{ $row->habilitado }}" <?php if($row->habilitado == 1) echo 'checked="checked"';?> >
					</td>
					<td>
					 	<div class="dropdown">
						  ...
						  <ul class="dropdown-menu">
							...
							@if($access['is_edit'] ==1)
							<li><a  href="{{ URL::to('afectacion/update/'.$row->itd_id.'?return='.$return) }}" class="tips" title="{{ Lang::get('core.btn_edit') }}"><i class="fa fa-edit "></i> {{ Lang::get('core.btn_edit') }} </a></li>
							@endif
						  </ul>
						</div>
